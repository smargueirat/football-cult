...
Also run \`python3 scripts/gear-mining/refresh_gear.py\` — same idea as refresh_boots.py but for src/data/gloves.ts and src/data/balls.ts (guantes de arquero y pelotas de fútbol), added 2026-09-17. Reuses the exact same feed cache (Foot-Store ES/FR, Sport is Good ES/FR, Deporte Outlet, Gigasport DE/CH/FR) via scripts/gear-mining/mine_gear.py -- no separate download step, no new feed sources. Deterministic ids same as boots (safe to re-run daily). git add src/data/gloves.ts and src/data/balls.ts alongside everything else. Print its summary too.
