...
// SIZES/brandNames/teamCategory/teamColors/teamFlags/teamNames/typeNames
// desde @/lib/productMeta (NO @/data/products): products.ts es el
// catálogo completo (5.9MB/76 mil líneas) -- importar cualquier símbolo
// real desde el barrel arrastra el archivo entero al cliente sin importar
// qué tan chico sea. `seasons` (la única lista derivada de verdad del
// array `products`) llega como prop, calculada en el server component
// (page.tsx). Este archivo es el contenido real del panel de filtros --
// FloatingFilterButton.tsx (el que se importa desde afuera) es solo un
// gate liviano que recién lo carga con dynamic()+ssr:false una vez que
// el usuario scrollea, así ni siquiera este chunk se pide en la carga
// inicial del home (mismo patrón que CompareBarContent en CompareBar.tsx).
import { SIZES, brandNames, teamCategory, teamColors, teamFlags, teamNames, typeNames } from "@/lib/productMeta";
...
export default function FloatingFilterButtonContent({ seasons }: { seasons: string[] }) {
