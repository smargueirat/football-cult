...
export async function generateMetadata({ params }: P): Promise<Metadata> {
  ...
  return gearMetadata({ section: "entrenamiento", brandSlug: brand }, asLocale(locale));
}

export default async function Page({ params }: P) {
  const { locale, brand } = await params;
  return <GearHub spec={{ section: "entrenamiento", brandSlug: brand }} locale={asLocale(locale)} />;
}
